...
local function createColorPicker(parent, yPos)
    ...
    colorPalette.Size = UDim2.new(0, 280, 0, 200)
    colorPalette.Position = UDim2.new(0, -260, 1, 2)
    ...
    -- Основная цветовая область (большая справа)
    local colorArea = Instance.new("Frame")
    colorArea.Size = UDim2.new(0, 180, 0, 120)
    colorArea.Position = UDim2.new(0, 70, 0, 30)
    ...
    -- Градиент насыщенности (горизонтальный: белый -> прозрачный)
    ...
    -- Градиент яркости (вертикальный: прозрачный -> черный)
    local brightnessOverlay = Instance.new("Frame")
    brightnessOverlay.Size = UDim2.new(1, 0, 1, 0)
    brightnessOverlay.BackgroundTransparency = 1
    ...
    -- Полоса оттенков (вертикальная слева)
    local hueBar = Instance.new("Frame")
    hueBar.Size = UDim2.new(0, 30, 0, 120)
    ...
    rainbowBar.Size = UDim2.new(0, 240, 0, 15)
    rainbowBar.Position = UDim2.new(0, 10, 0, 160)
    rainbowBar.BorderSizePixel = 1
    rainbowBar.BorderColor3 = colors.border
    rainbowBar.ZIndex = 26
    rainbowBar.Parent = colorPalette
    ...
    rainbowGradient.Parent = rainbowBar
    
    -- Индикатор на цветовой области
    local colorIndicator = Instance.new("Frame")
    colorIndicator.Size = UDim2.new(0, 10, 0, 10)
    colorIndicator.Position = UDim2.new(0.8, -5, 0.2, -5)
    ...
    -- Индикатор на радужной полосе
    local rainbowIndicator = Instance.new("Frame")
    rainbowIndicator.Size = UDim2.new(0, 3, 1, 4)
    rainbowIndicator.Position = UDim2.new(0, 0, 0, -2)
    rainbowIndicator.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
    rainbowIndicator.BorderSizePixel = 1
    rainbowIndicator.BorderColor3 = Color3.fromRGB(0, 0, 0)
    rainbowIndicator.ZIndex = 30
    rainbowIndicator.Parent = rainbowBar
    ...
    rgbLabel.Size = UDim2.new(0, 100, 0, 20)
    rgbLabel.Position = UDim2.new(0, 70, 1, -25)
    ...
    autoColorButton.Size = UDim2.new(0, 70, 0, 20)
    autoColorButton.Position = UDim2.new(1, -80, 1, -25)
    ...
    local function updateColor(h, s, v)
        ...
        colorIndicator.Position = UDim2.new(s, -5, 1-v, -5)
        
        -- Обновляем позицию индикатора на радужной полосе
        rainbowIndicator.Position = UDim2.new(h, -1.5, 0, -2)
    ...
    -- Обработка радужной полосы
    local rainbowButton = Instance.new("TextButton")
    rainbowButton.Size = UDim2.new(1, 0, 1, 0)
    rainbowButton.Text = ""
    rainbowButton.BackgroundTransparency = 1
    rainbowButton.ZIndex = 29
    rainbowButton.Parent = rainbowBar
    
    local rainbowDragging = false
    
    rainbowButton.MouseButton1Down:Connect(function()
        rainbowDragging = true
        print("Начало перетаскивания радужной полосы")
    end)
    
    UserInputService.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.MouseButton1 then
            rainbowDragging = false
        end
    end)
    
    rainbowButton.MouseMoved:Connect(function()
        if rainbowDragging then
            local mouse = UserInputService:GetMouseLocation()
            local relativeX = math.clamp((mouse.X - rainbowBar.AbsolutePosition.X) / rainbowBar.AbsoluteSize.X, 0, 1)
            updateColor(relativeX, currentSaturation, currentValue)
        end
    end)
    ...
    rainbowButton.MouseButton1Click:Connect(function()
        ...
        local relativeX = math.clamp((mouse.X - rainbowBar.AbsolutePosition.X) / rainbowBar.AbsoluteSize.X, 0, 1)
        updateColor(relativeX, currentSaturation, currentValue)
        print("Клик по радужной полосе:", relativeX)
    end)
    ...
    colorAreaButton.MouseButton1Down:Connect(function()
        ...
        print("Начало перетаскивания цветовой области")
    end)
    ...
    colorAreaButton.MouseMoved:Connect(function()
    ...
    colorAreaButton.MouseButton1Click:Connect(function()
        ...
        updateColor(currentHue, saturation, value)
        print("Клик по цветовой области:", relativeX, relativeY)
    end)
...
print("Skeet menu с исправленной цветовой палитрой загружен!")
